...
class Custom_CE_Loss(tf.keras.losses.Loss):
    ...
    def lossF(self):        
        ...
        for i in range(1,limitLoop,1):
            dummyX1 = x1Val[:,:,i-1] + (tf.math.multiply((self.y_pred2[:,:,0]),x1Val[:,:,i-1])-tf.math.multiply((0.1*self.y_pred2[:,:,1]),tf.math.multiply(x1Val[:,:,i-1],x2Val[:,:,i-1])))*tau
            dummyX2 = x2Val[:,:,i-1] + (tf.math.multiply((self.y_pred2[:,:,2]),tf.math.multiply(x1Val[:,:,i-1],x2Val[:,:,i-1]))-tf.math.multiply((self.y_pred2[:,:,3]),x2Val[:,:,i-1])+self.y_ins[:,:,i-1])*tau 
            ...
            dummyX2 = tf.cond(sumDiff > 0.0, lambda: self.y_true2[:,:,i]+10, lambda: dummyX2)
            dummyX1 = tf.cond(sumDiff > 0.0, lambda: self.y_meal[:,:,i]+10, lambda: dummyX1)
        ...
        #print(xValD2)
        err =  tf.math.reduce_sum(tf.math.square(self.y_true2[:,:,0:limitLoop]-x2Val[:,:,0:limitLoop])/limitLoop,axis=2)
...
class HarModel:
    ...
    def __init__(self,model_type,model_size,learning_rate = 0.001):
        ...
        self.x = tf.compat.v1.placeholder(dtype=tf.float32,shape=[None,None,501])
        self.ins = tf.compat.v1.placeholder(dtype=tf.float32,shape=[None,None,501])
        self.mL = tf.compat.v1.placeholder(dtype=tf.float32,shape=[None,None,501])
        self.target_y = tf.compat.v1.placeholder(dtype=tf.float32,shape=[None,None,501])
        self.basal = tf.compat.v1.placeholder(dtype=tf.float32,shape=[None,None,501])
