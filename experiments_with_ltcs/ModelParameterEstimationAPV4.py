...
class Custom_CE_Loss(tf.keras.losses.Loss):
    ...
    def lossF(self):        
        ...
        xVal = tf.keras.Input(dtype=tf.float32,shape=[None,None,1])    
        GVal = self.y_true2[:,:,0]
        ...
        err = tf.keras.Input(dtype=tf.float32,shape=[None,None])
...
class HarData:
    ...
    def __init__(self,seq_len=16):
        ...
        train_x = np.loadtxt("data/har/UCI HAR Dataset/train/TrainDataAPVR3.txt")
        train_y = (np.loadtxt("data/har/UCI HAR Dataset/train/TrainDataAPVR3.txt")-1)#.astype(np.int32)
        train_ins = np.loadtxt("data/har/UCI HAR Dataset/train/TrainDataAPInsulinVR3.txt")
        train_meal = np.loadtxt("data/har/UCI HAR Dataset/train/TrainDataAPMealVR3.txt")
        train_basal = np.loadtxt("data/har/UCI HAR Dataset/train/TrainDataAPBasalVR3.txt")
        train_meal2 = train_meal

        test_x = np.loadtxt("data/har/UCI HAR Dataset/test/TestDataAPVR3.txt")
        test_y = (np.loadtxt("data/har/UCI HAR Dataset/test/TestDataAPVR3.txt")-1)#.astype(np.int32)
        test_ins = np.loadtxt("data/har/UCI HAR Dataset/test/TestDataAPInsulinVR3.txt")
        test_meal = np.loadtxt("data/har/UCI HAR Dataset/test/TestDataAPMealVR3.txt")
        test_meal2 = test_meal
        test_basal = np.loadtxt("data/har/UCI HAR Dataset/test/TestDataAPBasalVR3.txt")
...
class HarModel:
    ...
    def __init__(self,model_type,model_size,learning_rate = 0.001):
        ...
        self.x = tf.keras.Input(dtype=tf.float32,shape=[None,None,200])
        self.ins = tf.keras.Input(dtype=tf.float32,shape=[None,None,200])
        self.mL = tf.keras.Input(dtype=tf.float32,shape=[None,None,200])
        self.target_y = tf.keras.Input(dtype=tf.float32,shape=[None,None,200])
        self.basal = tf.keras.Input(dtype=tf.float32,shape=[None,None,200])
        ...
        if(model_type == "lstm"):
            ...
            head,_ = tf.keras.layers.RNN(self.fused_cell,head,dtype=tf.float32,time_major=True)
            ...
            # rnn_layer = tf.keras.layers.RNN(ltc.LTCCell(model_size),dtype=tf.float32,time_major=True,
            #                                 return_sequences=True,
            #                                 return_state=True
            #                                 )
            # Initialize the initial state
            # initial_hidden_state = tf.zeros([16, model_size], dtype=tf.float32)
            # initial_cell_state = tf.zeros([16, model_size], dtype=tf.float32)
            # initial_state = (initial_hidden_state, initial_cell_state) 
            # head,_ = rnn_layer(head, initial_state = initial_state)
...
# #########
        ...
        #      self.loss = tf.reduce_mean(Custom_CE_Loss(labels = self.target_y,logits = self.y,insulin=self.ins,meal=self.mL,basal=self.basal).lossF())
        # grads = tape.gradient(self.loss, lossVal)
        # # Ask the optimizer to apply the gradients on variables.
        # optimizer.apply_gradients(zip(grads, lossVal))



        self.train_step = optimizer.compute_gradients(self.loss, lossVal, tape = tf.GradientTape())
        # self.train_step = optimizer.minimize(self.loss, lossVal)
    ...
    def fit(self,gesture_data,epochs,verbose=True,log_period=50):
        ...
        for e in range(epochs):
            ...
            for batch_x,batch_y,batch_ins,batch_mL,batch_basal in gesture_data.iterate_train(batch_size=16):
                ...
                # acc,loss, t_y = self.sess.run([self.accuracy,self.loss,self.y],{self.x:batch_x,self.target_y: batch_y, self.ins: batch_ins, self.mL: batch_mL, self.basal: batch_basal}) 
                print("ty:".format(t_y))
