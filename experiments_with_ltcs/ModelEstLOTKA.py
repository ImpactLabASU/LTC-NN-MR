...
class Custom_CE_Loss(tf.keras.losses.Loss):
    ...
    def lossF(self):        
        ...
        for i in range(1,limitLoop,1):
            dummyX1 = x1Val[:,:,i-1] + (tf.math.multiply((0.2 + 0.6*self.y_pred2[:,:,0]),x1Val[:,:,i-1])-tf.math.multiply((0.01+0.03*self.y_pred2[:,:,1]),tf.math.multiply(x1Val[:,:,i-1],x2Val[:,:,i-1])))*tau
            dummyX2 = x2Val[:,:,i-1] + (tf.math.multiply((0.002+0.006*self.y_pred2[:,:,2]),tf.math.multiply(x1Val[:,:,i-1],x2Val[:,:,i-1]))-tf.math.multiply((0.2+0.6*self.y_pred2[:,:,3]),x2Val[:,:,i-1])+self.y_ins[:,:,i-1])*tau
