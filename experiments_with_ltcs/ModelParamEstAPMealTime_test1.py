...
class Custom_CE_Loss(tf.keras.losses.Loss):
    ...
    def lossF(self):        
        ...
        gVal = tf.expand_dims(gVal,2)
        newMeal = tf.compat.v1.placeholder(dtype=tf.float32,shape=[None,None,1])
        newMealV2 = tf.compat.v1.placeholder(dtype=tf.float32,shape=[None,None,1])
        mealStartInd = tf.math.floor(self.y_pred2[:,:,10]*200)
        mealDur = 40
        

        mealStartInd = tf.clip_by_value(mealStartInd,clip_value_min=0,clip_value_max=200-mealDur)
        mealStartInd = tf.cast(mealStartInd,tf.int32)
        
        for i in range(0,200,1):
            newArray = tf.cast(tf.equal(mealStartInd,i),tf.float32)
            newArray = tf.multiply(self.y_meal[:,:,0],newArray)
            if i == 0 :
                newMeal = newArray
                newMeal = tf.expand_dims(newMeal,2)
            else:
                newArray = tf.expand_dims(newArray,2)
                newMeal = tf.concat([newMeal,newArray],2)
                
        mealEndInd = mealStartInd + mealDur

        mealEndInd = tf.cast(mealEndInd,tf.int32)

        for i in range(0,200,1):
            newArray = tf.cast(tf.equal(mealEndInd,i),tf.float32)
            newArray = tf.multiply(-self.y_meal[:,:,0],newArray)
            if i == 0:
                newMealV2 = newArray
                newMealV2 = tf.expand_dims(newMealV2,2)
            else:
                newArray = tf.expand_dims(newArray,2)
                newMealV2 = tf.concat([newMealV2,newArray],2)

        newMeal = tf.math.add(newMeal,newMealV2)        
        ...
        for i in range(1,limitLoop,1):
            ...
            dummyH = hVal[:,:,i-1]+tau*(-2*tf.math.multiply((0.004+0.4*self.y_pred2[:,:,6]),hVal[:,:,i-1])-tf.math.multiply(tf.math.square((0.004+0.4*self.y_pred2[:,:,6])),gVal[:,:,i-1])+tf.math.multiply((0.02+0.1*self.y_pred2[:,:,7]),newMeal[:,:,i-1]))
...
class HarData:
    ...
    def __init__(self, i, seq_len=16):
        ...
        train_y = (np.loadtxt(f"data/har/train_test_combined/TrainTestLNNData/Preg1_TrainDataAPVR3_{i}.txt")-1)#.astype(np.int32)
        train_ins = np.loadtxt(f"data/har/train_test_combined/TrainTestLNNData/Preg1_TrainDataAPInsulinVR3_{i}.txt")
        train_meal = np.loadtxt(f"data/har/train_test_combined/TrainTestLNNData/Preg1_TrainDataAPMealVR3_{i}.txt")
        train_basal = np.loadtxt(f"data/har/train_test_combined/TrainTestLNNData/Preg1_TrainDataAPBasalVR3_{i}.txt")
        train_meal2 = train_meal

        test_x = np.loadtxt(f"data/har/train_test_combined/TrainTestLNNData/Preg1_TestDataAPVR3_{i}.txt")
        test_y = (np.loadtxt(f"data/har/train_test_combined/TrainTestLNNData/Preg1_TestDataAPVR3_{i}.txt")-1)#.astype(np.int32)
        test_ins = np.loadtxt(f"data/har/train_test_combined/TrainTestLNNData/Preg1_TestDataAPInsulinVR3_{i}.txt")
        test_meal = np.loadtxt(f"data/har/train_test_combined/TrainTestLNNData/Preg1_TestDataAPMealVR3_{i}.txt")
        test_meal2 = test_meal
        test_basal = np.loadtxt(f"data/har/train_test_combined/TrainTestLNNData/Preg1_TestDataAPBasalVR3_{i}.txt")
...
class HarModel:
    ...
    def fit(self,gesture_data,epochs,verbose=True,log_period=50,i=None):
        ...
        for e in range(epochs):
            ...
            losses2_result = sum(losses2)/len(losses2)
            losses2_result_array = np.array([losses2_result])    

            # if e == 198:
            file_path = f'result_APMealTime/result_losses2_result_ltc{i}.txt'
            # Save the array to the file
            np.savetxt(file_path, losses2_result_array, fmt='%f')
            ...
            # if e == 198:
            # with open('result_APMealTime/result_ty_result{i}.txt', 'ab') as f1:
            #     np.savetxt('result_APMealTime/result_ty_result{i}.txt', tyMean2_result, fmt='%f')
            #     f1.write(b'\n')
            file_path1 = f'result_APMealTime/result_ty_result_ltc{i}.txt'
            np.savetxt(file_path1, tyMean2_result, fmt='%f') 
            with open(file_path, 'a') as f:
                f.write('\n') 
            # Also save sum(losses2)/len(losses2)
...
if __name__ == "__main__":
    ...
    # strategy = tf.compat.v1.distribute.MirroredStrategy(devices=["GPU:0", "GPU:1"])
    # print("Number of devices: {}".format(strategy.num_replicas_in_sync))
    for i in range(1, 2):
        # sess = tf.compat.v1.InteractiveSession()  # Create an interactive session
        
        K.clear_session()
        print("current file number: ", i)

        begin = time.perf_counter()
        har_data = HarData(i)

        # with strategy.scope():
        # trainingTiming = TimingCallback()
        model = HarModel(model_type = args.model, model_size=args.size)

        model.fit(har_data,epochs=args.epochs,log_period=args.log, i = i)

        print(model.y)

        # print("Training_Time", sum(trainingTiming.logs))
