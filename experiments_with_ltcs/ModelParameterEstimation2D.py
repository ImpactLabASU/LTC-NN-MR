...
class Custom_CE_Loss(tf.keras.losses.Loss):
    ...
    def lossF(self):        
        ...
        for i in range(1,limitLoop,1):
            dummy1 = xHid[:,:,i-1] - tf.math.multiply((self.y_pred2[:,:,0]),xHid[:,:,i-1])*tau+inputX[0,i]*(self.y_pred2[:,:,1])*tau
...
class HarData:
    ...
    def __init__(self,seq_len=16):
        ...
        train_x = np.loadtxt("data/har/UCI HAR Dataset/train/new2DTrainV3.txt")
        train_y = (np.loadtxt("data/har/UCI HAR Dataset/train/new2DTrainV3.txt")-1)#.astype(np.int32)
        
        test_x = np.loadtxt("data/har/UCI HAR Dataset/test/new2DTestV3.txt")
        test_y = (np.loadtxt("data/har/UCI HAR Dataset/test/new2DTestV3.txt")-1)#.astype(np.int32)
