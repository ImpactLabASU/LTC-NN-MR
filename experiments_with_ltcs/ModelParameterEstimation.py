...
def evaluate_loss(labels: tf.Tensor, logits: tf.Tensor)->tf.Tensor:
    overErr = 0;
    
    for s in range(0,labels.shape[0],1):
        xVal[0] = labels[s,0]
        err = 0
        for j in range(1,labels.shape[1],1):
            xVal[j] = xVal[j-1] - logits[0]*(1-xVal[j-1])**2*0.1 - logits[1]*xVal[j-1]*0.1
            err = err + (labels[s,j]-xVal[j])**2
        err = (err/561)**0.5
        overErr[s] =  err
...
class Custom_CE_Loss(tf.keras.losses.Loss):
    def __init__(self,labels,logits):
        self.y_true2 = labels
        self.y_pred2 = logits
        #self.yDummy = tf.tile(self.y_pred2,561)
    ...
    def lossF(self):        
        ...
        for i in range(1,limitLoop,1):
            dummy = xVal[:,:,i-1]-tf.math.multiply(self.y_pred2[:,:,0],1-tf.math.square(xVal[:,:,i-1]))*0.1 - tf.math.multiply(self.y_pred2[:,:,1],xVal[:,:,i-1])*0.1
        ...
        #yDummy = tf.reshape(tf.tile(yDummy,[1, 1, 1, 561]),[tf.shape(yDummy)[0],tf.shape(yDummy)[1],tf.shape(yDummy)[2],561])
        ...
        #yDummy = tf.reshape(tf.tile(self.y_pred2,561),[tf.shape(self.y_pred2)[0],tf.shape(self.y_pred2)[1],tf.shape(self.y_pred2)[2],561]) 
    ...
    def lossFV2(self):        
        ...
        #breakpoint() 
        for i in range(1,561,1):
            dummy = xVal[:,:,i-1]-tf.math.multiply(self.y_pred2[:,:,0],xVal[:,:,i-1])*0.1
            dummy = tf.expand_dims(dummy,2)
            xVal = tf.concat([xVal,dummy],2)
        #xVal2[:,:,0].assign(self.y_true2[:,:,0])
        ...
        #print(xValD2)
        err =  tf.math.reduce_sum(tf.math.square(self.y_true2-xVal)/561,axis=2)
        err = tf.math.sqrt(err)
...
class HarData:
    ...
    def __init__(self,seq_len=16):
        ...
        train_x = np.loadtxt("data/har/UCI HAR Dataset/train/newXtrainV3.txt")
        train_y = (np.loadtxt("data/har/UCI HAR Dataset/train/newXtrainV3.txt")-1)#.astype(np.int32)
        
        test_x = np.loadtxt("data/har/UCI HAR Dataset/test/newXtestV3.txt")
        test_y = (np.loadtxt("data/har/UCI HAR Dataset/test/newXtestV3.txt")-1)#.astype(np.int32)
...
class HarModel:
    ...
    def __init__(self,model_type,model_size,learning_rate = 0.001):
        ...
        self.target_y = tf.compat.v1.placeholder(dtype=tf.float32,shape=[None,None,561])
        ...
        self.y = tf.compat.v1.layers.Dense(2,activation='sigmoid')(head) # Dense layer output should be same as the number of model parameter 
        print("logit shape: ")
        ...
        #    logits = self.y,
        #))
        lossVal = Custom_CE_Loss(labels = self.target_y,logits = self.y).lossF() 
        print("lossVal {}".format(lossVal))

        self.loss = tf.reduce_mean(Custom_CE_Loss(labels = self.target_y,logits = self.y).lossF())
    ...
    def fit(self,gesture_data,epochs,verbose=True,log_period=50):
        ...
        for e in range(epochs):
            if(e%log_period == 0):
                ...
                test_acc,test_loss = self.sess.run([self.accuracy,self.loss],{self.x:gesture_data.test_x,self.target_y: gesture_data.test_x})
                valid_acc,valid_loss = self.sess.run([self.accuracy,self.loss],{self.x:gesture_data.valid_x,self.target_y: gesture_data.valid_x})
            ...
            for batch_x,batch_y in gesture_data.iterate_train(batch_size=16):
                acc,loss,t_step,t_y = self.sess.run([self.accuracy,self.loss,self.train_step,self.y],{self.x:batch_x,self.target_y: batch_y})
                #breakpoint() 
